...
const Session = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                const slotRes = await axios.get(
                    `${AVAIL_SERVICE}/availability/slots/${slotId}`
                );
                ...
                if (slotData?.mentorId) {
                    const mentorRes = await axios.get(
                        `${SEARCH_SERVICE}/mentors/${slotData.mentorId}`
                    );
                    setMentor(mentorRes.data?.mentor);
                }

                const filesRes = await axios.get(
                    `${FILE_SERVICE}/files/${slotId}`
                );
                setFiles(
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            const res = await axios.post(`${FILE_SERVICE}/files`, payload);
    ...
    const handleDelete = async (fileId: string) => {
        if (!window.confirm("Are you sure you want to delete this file?"))
            return;
        try {
            await axios.delete(`${FILE_SERVICE}/files/${fileId}`);
